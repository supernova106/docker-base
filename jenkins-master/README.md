...
Jenkins Master Docker Images with customized plugins.
...
- Prod, to be deployed on Kubernetes
...
- Using single container on EC2 instance

```
./jenkins-master/scripts/run-jenkins.sh
```
...
sudo docker service create \
    ...
    --mount type=volume,volume-driver=cloudstor:aws,source={{.Service.Name}}-{{.Task.Slot}}-vol,destination=/var/jenkins_home,volume-opt=backing=relocatable,volume-opt=size=20,volume-opt=ebstype=gp2 \
    jenkins/jenkins:lts
```
...
Set up jenkins secret

```
echo "-master http://jenkins:8080 -password <PASSWORD> -username admin" | sudo docker secret create jenkins-v1 -
```
